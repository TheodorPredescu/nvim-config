...
local function isLazyGitTerminal()
	local bufferName = vim.api.nvim_buf_get_name(0):lower()
	return vim.bo.filetype == "lazygit" or string.find(bufferName, "lazygit") ~= nil
end

vim.keymap.set("i", "<C-c>", "<Esc>")
vim.keymap.set("n", "<leader>o", ":Oil<CR>", { desc = "Exit terminal mode" })
vim.keymap.set("t", "<C-[>", function()
	if isLazyGitTerminal() then
		feedKey("<Esc>")
		return
	end
